...
# Rendered on demand rather than read from data/planets/Earth.webp. That file
# is 288px -- sized for the 140 CSS px the wiki draws a disc at -- and the card
# draws it at 720, so reading it meant upscaling 2.5x on the single image every
# shared link unfurls as. GLOBE_PX is rendered directly instead, at twice the
# raster for the same limb antialiasing the served discs get.
GLOBE_PLANET = "Earth"
GLOBE_PX = 720
...
def globe_source() -> Image.Image:
    """The planet disc, rendered at the size the card draws it.

    This is the one part of the card that needs the game installed, because
    planet_render reads the spheroid binaries out of it. The card is a
    committed artifact regenerated by hand when the design changes, so paying
    that cost here is better than shipping a soft globe.
    """
    from generator import planet_render as pr

    tmp = os.path.join(ROOT, "public", ".og-globe.tmp.webp")
    try:
        if not pr.render(GLOBE_PLANET, tmp, size=GLOBE_PX * 2, fit=GLOBE_PX):
            raise SystemExit(
                f"could not render {GLOBE_PLANET}: the game files are needed here.\n"
                f"See tools/README.md -- planet_render reads the spheroid .dat "
                f"binaries from the install named in tools/.env."
            )
